...
-e DISPLAY_N=:0 \
-e WORLD_NAME=Spain_track \
-e NUMBER_OF_TRIALS=5 \
-e MODEL_S3_PREFIX=$DR_LOCAL_S3_MODEL_PREFIX \
-e MODEL_S3_BUCKET=larsll-deepracer-eu-4 \
-e APP_REGION=eu-central-1 \
-e S3_YAML_NAME=eval-params.yaml \
-e KINESIS_VIDEO_STREAM_NAME=None \
-e ENABLE_KINESIS=False \
-e ENABLE_GUI=False \
-e ROLLOUT_IDX=0 \
-v /tmp/.X11-unix/X0:/tmp/.X11-unix/X0:ro \
-v /etc/localtime:/etc/localtime:ro \
awsdeepracercommunity/deepracer-robomaker:gpu-test "${ROBOMAKER_COMMAND}"
